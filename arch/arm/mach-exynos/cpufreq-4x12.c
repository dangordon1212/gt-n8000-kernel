...
static unsigned int get_max_cpufreq_idx(void)
{
	...
	pr_info("%s: adjusting index to 0 from %d\n", __func__, index);
	index = L0;
#endif
	return index;
}
...
static void __init set_volt_table(void)
{
	...
	pr_debug("DVFS : is4212 %s is4412 %s armclk_max %u samsung_rev 0x%x\n",
		soc_is_exynos4212() ? "t" : "f",
		soc_is_exynos4412() ? "t" : "f",
		exynos_armclk_max, samsung_rev());
